#!/usr/bin/env python3
"""
feature feature search (InterPLM-style) for HyenaDNA + SAE (layer 5), FAR-BACKGROUND per-base evaluation.

Goal
- Sample n_pos positive 2kb windows centered on a feature interval midpoint.
- Sample n_neg negative 2kb windows with NO overlap with any feature interval.
- Extract HyenaDNA layer token embeddings (1 base = 1 token).
- Compute SAE feature activations for:
    * all bases within feature intervals inside positive windows (positives)
    * EXACT matched-count random bases from negative windows (negatives)
- Compute per-base feature–concept association (precision + recall + best-F1 over thresholds).

Inputs
- hg38 FASTA
- feature BED (0-based, half-open)
- SAE checkpoint trained on HyenaDNA embeddings for the chosen layer
- HyenaDNA model id

Outputs
- out_dir/feature_eval_summary.csv : per-feature best threshold + precision/recall/F1 + confusion counts
- out_dir/feature_eval_details.pt  : saved tensors for further analysis (optional)

Usage example
python feature_feature_search_perbase_farbg.py \
  --fasta data/hg38.primary.fa \
  --feature_bed data/annotations/gencode/feature_TSS_1000up_100down.bed \
  --sae_ckpt runs/sae/<your_run>/ckpt_step_XXXXXXX.pt \
  --out_dir runs/feature_feature_search \
...
from sae_models.relu import SparseAutoencoder
...
class WindowSample:
    ...
    # For positive windows, this is the specific feature interval used to center the window.
...
def sample_positive_windows_centered(
    ...
    if len(out) < n_pos:
        raise RuntimeError(f"Could only sample {len(out)}/{n_pos} positive windows (check feature BED / contigs).")
    return out


def sample_negative_windows_no_feature(
    feature_trees: Dict[str, IntervalTree],
    chrom_sizes: Dict[str, int],
    seq_len: int,
    n_neg: int,
) -> List[WindowSample]:
    ...
    while len(out) < n_neg and attempts < max_attempts:
        ...
        if any_overlap(feature_trees, chrom, ws, we):
            continue
...
# ----------------------------
# Collect activations for positives (feature bases) and negatives (random bases)
# ----------------------------
@torch.inference_mode()
...
def feature_mask_for_window(
    feature_trees: Dict[str, IntervalTree],
    w: WindowSample,
    seq_len: int,
) -> torch.Tensor:
    """Returns boolean mask [L] for bases in ANY feature interval overlapping the window."""
    hits = intersect_intervals(feature_trees, w.chrom, w.start, w.end)
...
def collect_activation_sets_farbg_matched(
    ...
    """
    FAR-background per-base evaluation:
      - Positives: all feature-overlapping bases inside positive windows
      - Negatives: EXACT same number of random bases sampled from feature-free windows
    Returns:
      A_pos: [NposBases, F] CPU
      A_neg: [NnegBases, F] CPU (NnegBases == NposBases)
      diagnostics: masks and sampled neg indices
    """
    # Build positive embedding list
    pos_masks: List[torch.Tensor] = []
    x_pos_list: List[torch.Tensor] = []
    pos_counts: List[int] = []

    for i, w in enumerate(win_pos):
        m = feature_mask_for_window(feature_trees, w, seq_len)
        c = int(m.sum().item())
        pos_masks.append(m)
        pos_counts.append(c)
        if c > 0:
            x_pos_list.append(emb_pos[i, m, :])  # [c, D]

    if not x_pos_list:
        raise RuntimeError("No feature-labeled bases found in sampled positive windows.")

    X_pos = torch.cat(x_pos_list, dim=0)  # CPU [NposBases, D]
    n_pos_bases = int(X_pos.shape[0])

    # Sample EXACT matched number of negatives uniformly across negative windows/bases
    nwin_neg = int(emb_neg.shape[0])
    if nwin_neg == 0:
        raise RuntimeError("No negative windows available.")

    neg_win_ids = [random.randrange(nwin_neg) for _ in range(n_pos_bases)]
    neg_pos_ids = [random.randrange(seq_len) for _ in range(n_pos_bases)]
    X_neg = emb_neg[torch.tensor(neg_win_ids), torch.tensor(neg_pos_ids), :]  # CPU [NposBases, D]

    # Encode with SAE on device, chunked
    ...
    A_neg = encode_in_chunks(sae, X_neg_dev)  # CPU [NposBases, F]
...
# ----------------------------
# Per-base association: threshold sweep, precision/recall, best-F1
# ----------------------------
...
def evaluate_feature_per_base(
    ...
    npos = int(a_pos.numel())
    nneg = int(a_neg.numel())
...
def run_feature_search_per_base(
    ...
    if npos != nneg:
        raise ValueError("This script expects matched counts: npos_bases == nneg_bases")
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for j in range(F_use):
            ...
            best = evaluate_feature_per_base(a_pos=a_pos_j, a_neg=a_neg_j, thresholds=thresholds)
...
def main() -> None:
    ...
    ap.add_argument("--feature_bed", required=True)
    ...
    ap.add_argument("--save_tensors", action="store_true", help="Save activations and sampling metadata to a .pt file")
    ...
    # Load feature intervals
    feature_rows = read_bed3(args.feature_bed)
    if not feature_rows:
        raise SystemExit(f"No rows in {args.feature_bed}")
    feature_trees = build_interval_trees(feature_rows)
    ...
    neg_windows = sample_negative_windows_no_feature(
    ...
    # Collect activation sets (far-background, matched counts)
    data = collect_activation_sets_farbg_matched(
        emb_pos=emb_pos,
        win_pos=pos_windows,
        emb_neg=emb_neg,
        win_neg=neg_windows,
        ...
        torch.save(
            {
                ...
                "pos_counts_per_window": data["pos_counts"],
            ...
            os.path.join(args.out_dir, "feature_eval_details.pt"),
        )
